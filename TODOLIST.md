...
- [ ] 使用 HCP 路径定义资源映射（优先级：高）  Owner: @arch
    - 验收标准：
        - 提供 HCP 路径命名规则与示例（表路径示意）。
...
- [ ] 统一资源定位规范（优先级：高）  Owner: @arch
    - 验收标准：
        - 明确资源定位流程：HCP 路径 -> 索引 -> 实际资源。
        - 提供至少两种查找策略示例（直接映射、模糊/前缀匹配）。
        - 编写失败回退策略与错误消息规范。
- [ ] 实现 HCP 路径查找工具/库（优先级：中）  Owner: @dev
    - 验收标准：
...
- M2（2-4 周）：实现基础 CLI（local deploy / 指定地址）、HCP 路径设计与库。
